...
## Build, test & check
- `cargo check` — verify compilation (run this for every change, not full tests)
...
- MQTT tests in `data::transception::mqtt::tests` are skipped unless broker is at `localhost:1883` (start via `docker compose up -d`)
...
- Currently `hyperliquid` and `dydx` are implemented
...
- `MemoryMap<V>` trait — `set(key, value)` / `get(key) -> Option<V>`
- `ExpirationBuffer<V>` trait — `add(value)` / `get() -> Option<Box<dyn Iterator<Item = V>>>` (accumulates values with TTL, returns boxed iterator)
- Factory: `memory_map::new(backend, cfg)` returns `Box<dyn MemoryMap<V>>`; `expiration_buffer::new(backend, ttl)` returns `Box<dyn ExpirationBuffer<V>>`
- Backends: `"native"` (in-memory HashMap), `"redis"` (with TTL, via unix socket)
- `NativeMemoryMap` and `RedisMemoryMap` implement `MemoryMap`; `NativeExpirationBuffer` implements `ExpirationBuffer`

### `Message` enum (in `data::types::message`)
...
- `OnceLock` + `Box<dyn MemoryMap>` is used for strategy state
